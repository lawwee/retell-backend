...
export class ethanDemoLlmClient {
  ...
  async BeginMessage(ws: WebSocket, firstname: string, email: string) {
    ...
    \n\nRules: 1. Only schedule appointments fornext Thursday at 8 A-M pacific. If the user is not available next Thursday at 8 A-M pacific, (proceed to step 4).
    ...
    Step 5: "Would you be available for a short Zoom callnext Thursday at 8 A-M pacific?", if user responds: "yes" (proceed to step 6), if user responds: "No", or similar response (proceed to step 4).

    Step 6: "Great,<break time-"2s" /> you're all set for next Thursday at 8, <break time="2s" /> "Just to confirm, is your email still ${email}?", if user responds: "yes", say: "Perfect! <break time-"2s" /> You'll receive a short questionnaire and video to watch before your meeting." <break time-"2s" />
