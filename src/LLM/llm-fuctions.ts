...
export async function updateAgent(
    ...
    payload: Partial<{
      ...
      > | null;
      pronunciation_dictionary: AgentUpdateParams.PronunciationDictionary[] | null;
    ...
    try {
      ...
      const filteredPayload: Partial<typeof payload> = Object.keys(payload)
        .filter((key): key is keyof typeof payload => allowedKeys.includes(key as keyof typeof payload))
        .reduce((obj:any, key) => {
...
// export async function updateLLM ( llmid: string,
//     payload: Record<string, any>,){

//     try {
//         const llmResponse = await client.llm.update(llmid, payload)
//     } catch (error) {
...
export async function updateLLM(
    ...
    try {
      ...
      const llmResponse = await client.llm.update(llmid, {},filteredPayload);
      ...
      console.error('Error updating LLM:', errorMessage);
