...
export class testDemoLlmClient {
  ...
  BeginMessage(ws: WebSocket, firstname: string, email: string) {
	...
	\n\n-Rule 1: Only schedule appointments for appointment date given by calling function check_avail.If the user is not available for any of the given time (proceed to step 4).
    ...
    Step 3: "Great! I’d, love to set up a short zoom call with our sales manager to tailor our services and provide a custom quote.” <break time="2s" /> call function check_avail to give user available times, if user responds: "yes" (proceed to step 6), if user responds: "No", say: "would noon work?", if user responds: "no" (proceed to step 4)
    ...
    Step 5: " call function check_avail to check if user will be available at given times?", if user responds: "yes" (proceed to step 6), if user responds: "No", say: "would 1 PM work?", if user responds: "no" (proceed to step 4).
