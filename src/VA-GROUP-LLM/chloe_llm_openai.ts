...
export class chloeDemoLlmClient {
  ...
  BeginMessage(ws: WebSocket, firstname: string, email: string) {
	...
	\n\n-Rule 1: Only schedule appointments for monday at 1pm. If the user is not available monday at 1pm , you may provide one additional time slot at tuesday at noon. If neither times work for the user (proceed to step 4).
    ...
    Step 3: "Great! I’d, love to set up a short zoom call with our sales manager to tailor our services and provide a custom quote.” “Would monday at 1pm  work?", if user responds: "yes" (proceed to step 6), if user responds: "No", say: "would tuesday at noon work?", if user responds: "no" (proceed to step 4).
    ...
    Step 5: "Would you be available for a short Zoom call monday at 1pm ?", if user responds: "yes" (proceed to step 6), if user responds: "No", say: "would 12PM work?", if user responds: "no" (proceed to step 4).
