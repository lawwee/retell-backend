...
export class danielDemoLlmClient {
  ...
  async BeginMessage(ws: WebSocket, firstname: string, email: string) {
    ...
    \n\nRules: 1. Only schedule appointments for next Monday at 9 AM. If the user is not available next Monday at 9 AM, (proceed to step 4).
    ...
    Step 5: "Would you be available for a short Zoom call next Monday at 9 AM ?", if user responds: "yes" (proceed to step 6), if user responds: "No", or similar response (proceed to step 4).

    Step 6: "Great,<break time-"2s" /> you're all set for next Monday at 9 AM, <break time="2s" /> "Can you please provide the best email to send the calendar invite to?", (After response) say: "Perfect! You'll receive a short questionnaire and video to watch before your meeting." (Wait for User's response, then continue)
