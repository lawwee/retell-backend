...
export class danielDemoLlmClient {
  ...
  async BeginMessage(ws: WebSocket, firstname: string, email: string) {
    ...
    \n\nRules: 1. Only schedule appointments for next wednesday 9 A-M. If the user is not available next wednesday 9 A-M, (proceed to step 4).
    ...
    Step 3: "Great! I’d, love to set up a short zoom call to tailor our services and create  custom quote for you.” <break time="2s" /> “Would next wednesday 9 AM work?", if user responds: "yes" (proceed to step 6), if user responds: "No", or similar objection (proceed to step 4).
    ...
    Step 5: "Would you be available for a short Zoom call next wednesday 9 A-M?", if user responds: "yes" (proceed to step 6), if user responds: "No", or similar response (proceed to step 4).

    Step 6: "Great,<break time-"2s" /> you're all set for next wednesday 9 A-M, <break time="2s" /> "Can you please provide the best email to send the calendar invite to?", (After response) say: "Perfect! You'll receive a short questionnaire and video to watch before your meeting." (Wait for User's response, then continue)
